...
@cash_bp.route("/edit/<int:log_id>", methods=["GET", "POST"])
@login_required
def edit(log_id):
    store_number = get_manager_store()
    if not store_number:
    ...
    log = CashLog.query.get_or_404(log_id)
    ...
    if request.method == "POST":
        ...
        if not shift_type:
            flash("Shift type is required.", "error")
            return redirect(url_for("cash.edit", log_id=log.id))
        ...
        log.total_cash = back_till + front_till + driver_banks
    ...
    return render_template(
        "cash.html",
        logs=logs,
        today_str=log.log_date.strftime("%Y-%m-%d"),
        store_number=store_number,
        manager_name=log.manager_name or session.get("user_name", ""),
        edit_log=log,
    )