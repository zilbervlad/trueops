...
def get_supervisor_stores():
    ...
    if role == "supervisor":
        return Store.query.filter_by(area_name=user_area, is_active=True).all()
...
def ensure_default_template():
    ...
    for i, (key, label, ftype) in enumerate(defaults, start=1):
        if key not in existing:
            db.session.add(VerificationTemplateField(
                field_key=key,
                field_label=label,
                field_type=ftype,
                sort_order=i,
...
def new_report():
    ...
    if request.method == "POST":
        ...
        for field in fields:
            ...
            db.session.add(VerificationReportValue(
                report_id=report.id,
        ...
        try:
            ...
            for field in fields:
                val = request.form.get(field.field_key, "")
